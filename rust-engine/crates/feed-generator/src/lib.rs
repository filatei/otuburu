...
impl Generator for BoomCrashGenerator {
    fn symbol(&self) -> &str { &self.cfg.symbol }
...
impl Generator for FxGenerator {
    ...
    fn next_tick(&mut self) -> Tick {
        ...
        self.s = self.s
            * ((self.cfg.mu - 0.5 * vc) * dt + (vc * dt).sqrt() * z1).exp();
        self.v = (self.v
...
mod tests {
    ...
    fn boom1000_ticks_are_deterministic() {
        let make = || BoomCrashGenerator::new(BoomCrashConfig {
            symbol: "BOOM1000".into(), seed: 11,
            start_price: 1000.0, sigma_small: 0.15,
            spike_mean: 25.0, spike_std: 10.0,
            spike_direction: 1.0, lambda_per_tick: 1.0 / 1000.0,
    ...
    fn spread_is_symmetric() {
        ...
        for _ in 0..50 {
            ...
            assert!((spread - 0.0001).abs() < 1e-5, "spread should be ~0.0001, got {spread}");
        }
    }
    ...
    fn crypto_regime_stays_bounded() {
        ...
        for _ in 0..1000 {
            let t = g.next_tick();
            assert!(t.mid > 0.0 && t.mid < 10_000_000.0, "price out of sane range: {}", t.mid);
        }
    }
}
