...
pub fn default_contract_specs() -> HashMap<String, ContractSpec> {
    [
        ("BOOM1000",  ContractSpec { contract_size: 1.0,       leverage: 100 }),
        ("BOOM500",   ContractSpec { contract_size: 1.0,       leverage: 100 }),
        ("CRASH1000", ContractSpec { contract_size: 1.0,       leverage: 100 }),
        ("frxEURUSD", ContractSpec { contract_size: 100_000.0, leverage: 500 }),
        ("frxGBPUSD", ContractSpec { contract_size: 100_000.0, leverage: 500 }),
        ("cryBTCUSD", ContractSpec { contract_size: 1.0,       leverage: 50  }),
        ("cryETHUSD", ContractSpec { contract_size: 1.0,       leverage: 50  }),
    ]
    .into_iter()
...
impl HouseStats {
    pub fn house_net(&self) -> f64 { -self.total_client_pnl }
    pub fn binary_win_rate(&self) -> Option<f64> {
        if self.binary_count == 0 { None }
        else { Some(self.binary_wins as f64 / self.binary_count as f64) }
    }
}
...
impl Book {
    ...
    pub fn margin_level(&self) -> f64 {
        ...
        if u == 0.0 { f64::INFINITY } else { self.equity() / u * 100.0 }
    }
    ...
    pub fn open_cfd(
        ...
        let q = self.quotes.get(symbol).ok_or_else(|| BookError::NoQuote(symbol.into()))?.clone();
        let spec = self.specs.get(symbol).ok_or_else(|| BookError::UnknownSymbol(symbol.into()))?;
        ...
        if margin > free {
            return Err(BookError::InsufficientMargin { need: margin, have: free });
        }
    ...
    pub fn close_cfd(&mut self, id: Uuid) -> Result<(CfdPosition, f64), BookError> {
        let p = self.positions.remove(&id).ok_or(BookError::PositionNotFound(id))?;
        let q = self.quotes.get(&p.symbol).ok_or_else(|| BookError::NoQuote(p.symbol.clone()))?.clone();
    ...
    fn pnl_for(&self, p: &CfdPosition, mark: f64) -> f64 {
        ...
        let cs = self.specs.get(&p.symbol).map(|s| s.contract_size).unwrap_or(1.0);
    ...
    pub fn open_binary(
        ...
        if stake <= 0.0 || !stake.is_finite() { return Err(BookError::InvalidStake(stake)); }
        if ticks == 0 || ticks > 200 { return Err(BookError::InvalidTicks(ticks)); }
        if stake > self.account.balance {
            return Err(BookError::InsufficientBalance {
                need: stake, have: self.account.balance,
            });
        }
        let q = self.quotes.get(symbol).ok_or_else(|| BookError::NoQuote(symbol.into()))?.clone();
    ...
    pub fn on_tick(&mut self, tick: &Tick) -> Vec<BinarySettlement> {
        ...
        for p in self.positions.values_mut().filter(|p| p.symbol == tick.symbol) {
        ...
        let to_settle: Vec<Uuid> = self.binaries
            .values()
        ...
        for id in to_settle {
            if let Some(b) = self.binaries.get_mut(&id) {
                ...
                if b.ticks_left == 0 {
                    ...
                    let payout = if won { b.stake * PAYOUT_MULTIPLIER } else { 0.0 };
                    self.account.balance += payout;
                    self.house.total_client_pnl += payout - b.stake;
                    if won { self.house.binary_wins += 1; }
                    settled.push(BinarySettlement { option: b, exit_mid: tick.mid, won, payout });
                }
            }
        }
        settled
    ...
    pub fn positions(&self) -> Vec<&CfdPosition> { self.positions.values().collect() }
    pub fn binaries(&self) -> Vec<&BinaryOption>  { self.binaries.values().collect() }
    pub fn quotes(&self) -> &HashMap<String, Tick> { &self.quotes }
}
